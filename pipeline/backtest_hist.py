...
def verify_against_archive(klines: dict[str, pd.DataFrame]) -> dict:
    ...
    注意归档取数用的是当日**最后一次 warmup 刷新的 K 线**：若归档早于收盘（历史上的
    盘中归档），最后一根就是**盘中**价格，与本机收盘后拉的最后一根会有差异。
    现行归档只在 15:35 收盘复核时写，而 close 阶段会先重跑 warmup（含当日完整 bar），
    所以这个差异应显著变小 —— 差异大小本身仍要如实报告，不预设它一定小。
    """
...
def render(panel, meta, w, combo, fac, rules, rel, mac, verify, stab,
    ...
    if verify:
        ...
        A("")
        A("> 差异来源已知：生产缓存的最后一根 K 线是**当天最后一次预热**时的价格 —— "
          "归档若早于收盘，那一根就是**盘中**价，本机是收盘后取的全日 bar，"
          "动量/KDJ 这类吃最新价的因子会因此有微小偏差。"
          "（现行归档只在 15:35 收盘复核时写，而 close 会先重跑 warmup，差异应更小。）")
