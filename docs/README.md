...
> **它吃的是「跨日」样本，不是「文件数」**。归档里每天会写两份：
> 整份 `YYYYMMDD_HHMM.json` 与轻量 `YYYYMMDD_HHMM_summary.json`（几 KB，含 11 维因子值，
> 开关见 `weights.yaml` 的 `archive` 段）。**同一天无论写几份都只算 1 天**，
...
| **权重为先验值** | 12 维权重尚未用真实历史数据校准。⚠️ 原计划「攒 2–4 周快照后跑 `backtest.py`」**方向是错的**：归档曾全是同一天的多份快照，按日期去重后样本数恒为 1。<br>**已解决一半**：`main.py` 每次 score 现会另写一份**分数矩阵 summary**（`archive.summary`），跨日样本会真正积累；`backtest.py` 也已能消费它并输出**逐因子 IC**。剩下的只是**等天数**。<br>同时**长历史重建法**（见第六节）已可用，但它只能覆盖 6 个可回溯维度 |
