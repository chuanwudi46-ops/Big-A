...
- 每个交易日自动评分 **14 次**：09:10 预热 + 09:35–15:05 每 30 分钟（**另加 09:45 / 13:25**，
  专为「09:50 / 13:30 开始操作」护航）+ 15:35 收盘复核并归档
- **板块评分视图**：12 维评分 + 宏观分组成决策矩阵，输出可执行动作而非单一信号
- **回撤 / 涨幅视图**：逐板块的 20/60/250 日从高点回撤、从低点反弹、区间位置
- **大盘关键位**：上证 / 沪深300 的**两套**支撑压力 —— ①**均线体系**（**5 / 10 / 20 / 30 / 60 日线 + 年线**，
  某条线是支撑还是压力**由现价在它上方还是下方决定**，另带近 5 日斜率与多空排列）
  ②日线·周线·月线·年线摆动前高 / 前低（日线档给出「位置区 ×N」，即同价位被测试次数）
- **事件日历**：未来 120 天的美联储议息 / 非农 / CPI·PCE / LPR / 政策会议 / 大额解禁，按优先级与类别筛选
- **主力资金**：沪深两市当日主力净流入（亿元）+ 超大 / 大 / 中 / 小四档拆解 + **自开盘累计**的分时曲线
  + 板块净流入 TOP10 / 净流出 TOP10（主力 = 大单 + 超大单）
...
```
GitHub Actions（免费定时执行，每交易日 14 次）
  09:10  warmup  抓历史K线增量 / 解禁日历 / 新闻池 / 财经日历（当天唯一一次）
  09:35…15:05    每 30 分钟一次 score（另加 09:45 / 13:25）：抓快照 -> 12维评分 ->
                 生成 JSON（大盘关键位 + 均线体系 + 事件日历 + 主力资金）
                 -> 提交 -> 重部署 Pages（**盘中不写归档**）
...
| 评分时点 / 刷新频率 | `.github/workflows/daily.yml` 的 cron（**UTC** 时间，北京时间 = UTC+8）。现为 **14 次/日**：09:10 预热 + 09:35–15:05 每 30 分钟 score（**另加 09:45 / 13:25**）+ 15:35 close。<br>分钟一般取 `:05`/`:35` 以避开 GitHub 的整点拥堵；**`09:50` / `13:30` 这两个操作时点各配两档**（09:35 + 09:45、13:05 + 13:25）—— cron 有 5–30 分钟延迟，**单档保证不了到点有数据**，靠前一档兜延迟、后一档保新鲜 |
| 归档时机 | `weights.yaml` 的 `archive.mode`（默认 **`on_close`**：只有 close 阶段写归档；设 `always` 则每次 score 都写，仓容约 680 MB/年） |
...
# 大盘关键位（日/周/月/年四档 + 均线体系 5/10/20/30/60 日线 / 年线）
...
> 盘中 score 不再写归档 —— 整份快照 200 KB × 14 次/日 ≈ 680 MB/年，
